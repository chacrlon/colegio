...
import persistence.UsuariosRolesFacadeLocal;
import utils.ConstantesUtils;
...
public class Controlador extends HttpServlet {
    ...
    @EJB
    private UsuariosRolesFacadeLocal usuariosRolesFacade;
    ...
    public static Boolean Rol_usuario(Integer nRol, HttpServletRequest request) {
        ...
        if (user != null) {
            ...
            if (listita.get(0).getRolesString().contains("Administrador")) {
                tipoUsuarioInteger = 1;
            } else if (listita.get(0).getRolesString().contains("siguiente rol")) {
                tipoUsuarioInteger = 2;
            } else if (listita.get(0).getRolesString().contains("siguiente rol")) {
                tipoUsuarioInteger = 3;
            }
        }
        if (tipoUsuarioInteger == nRol) {
            sal = true;
        }
        return sal;
    }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (menu.equals("Usuario")) {
                ...
                switch (accion) {
                    case "Listar":
                        List<models.Usuarios> usuariosList = usuariosFacade.findAll();
                        List<Status> statusList = statusFacade.findAll();
                        List<String> rolesList = ConstantesUtils.RolesList.getRolesList();
                        ListRolesUsuariosUtil.setUsuariosRolesString(usuariosList);
                    ...
                    case "Agregar":
                        ...
                        if (usuariosFacade.verifyUserExists(nicku)) {
                            request.setAttribute("message", ConstantesUtils.MensajesList.ADVERTENCIA+ " El usuario ya existe.");
                            request.getRequestDispatcher("Controlador?menu=Usuario&accion=Listar").forward(request, response);
                            return;
                        }
                        models.Usuarios usuario = new models.Usuarios();
                        models.Status status = statusFacade.find(Integer.valueOf(request.getParameter("Status")));
                        usuario.setNick(nicku);
                        usuario.setPassword(passu);
                        usuario.setStatusIdStatus(status);
                        models.UsuariosRoles ur = new UsuariosRoles();
                        ur.setDescripcion(request.getParameter("rol"));
                        ur.setUsuariosIdUsuario(usuario);
                        List<UsuariosRoles> urlist = new ArrayList();
                        urlist.add(ur);
                        usuario.setUsuariosRolesList(urlist);
                        usuariosFacade.create(usuario);
                        ur.setDescripcion(request.getParameter("rol"));
                        // usuariosRolesFacade.create(ur);
                        request.setAttribute("message", ConstantesUtils.MensajesList.EXITO+" El usuario se ha guardado correctamente");
                        request.getRequestDispatcher("Controlador?menu=Usuario&accion=Listar").forward(request, response);
                        break;
                    case "Editar":
                        id_usuario = Integer.parseInt(request.getParameter("id"));
                        //Usuarios a=usuariosCRUD.listarId(id_usuario);
                        models.Usuarios usuari = usuariosFacade.find(id_usuario);
                        usuarios = usuari;
                        request.setAttribute("user", usuari);
                        request.setAttribute("idUsuario", usuari.getIdUsuario());
                        request.getRequestDispatcher("Controlador?menu=Usuario&accion=Listar").forward(request, response);
                        request.getRequestDispatcher("usuario.jsp").forward(request, response);
                        break;
                    case "Actualizar":
...
//                    usuarios = usuariosFacade.find(id_usuario);
                        models.Status sta = statusFacade.find(Integer.valueOf(request.getParameter("Status")));
                        String nicku1 = request.getParameter("nicku");
                        String passu1 = request.getParameter("passu");
                        usuarios.setNick(nicku1);
                        usuarios.setPassword(passu1);
                        usuarios.setStatusIdStatus(sta);
                        Boolean tieneRol = usuariosRolesFacade.userHasRol(usuarios.getIdUsuario(), request.getParameter("rol"));
                        if (!tieneRol) {
                            models.UsuariosRoles userR = new UsuariosRoles();
                            userR.setUsuariosIdUsuario(usuarios);
                            userR.setDescripcion(request.getParameter("rol"));
                            usuariosRolesFacade.create(userR);
                            if (usuarios.getUsuariosRolesList() != null) {
                                usuarios.getUsuariosRolesList().add(userR);
                            } else {
                                List<UsuariosRoles> url = new ArrayList();
                                url.add(userR);
                                usuarios.setUsuariosRolesList(url);
                            }
                        }
                           usuariosFacade.edit(usuarios);
                          request.setAttribute("message", ConstantesUtils.MensajesList.EXITO+" El usuario se ha actualizado correctamente.");
                         request.getRequestDispatcher("Controlador?menu=Usuario&accion=Listar").forward(request, response);
                        break;
                    case "Eliminar":
                        id_usuario = Integer.parseInt(request.getParameter("id"));
                        ...
                        request.setAttribute("message", ConstantesUtils.MensajesList.EXITO+" El usuario se ha eliminado correctamente.");
                        request.getRequestDispatcher("Controlador?menu=Usuario&accion=Listar").forward(request, response);
                        break;
                    default:
                        throw new AssertionError();
